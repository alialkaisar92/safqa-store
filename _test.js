...
function loadTheme(id){var s=document.createElement('script');s.src='/premium.js';s.onload=function(){TH=window.THEME;document.getElementById('themeCss').textContent=TH.css||'';if(TH.cssUrl){var l=document.createElement('link');l.rel='stylesheet';l.href=TH.cssUrl;document.head.appendChild(l);}render();};s.onerror=function(){TH=window.THEME;render();};document.head.appendChild(s);}
function cards(idx){return idx.map(function(i){return TH.card(P[i],i)}).join('')}
function wrap(t,inner,cls){return '<section class="'+(cls||TH.wrapClass||'sec-wrap')+'"><h2 class="'+(TH.titleClass||'sec-t')+'">'+t+'</h2><div class="'+(TH.gridClass||'pgrid')+'">'+inner+'</div></section>'}
...
function secHtml(id){
 ...
 if(id=='products')return '<section class="'+(TH.wrapClass||'sec-wrap')+'"><h2 class="'+(TH.titleClass||'sec-t')+'" id="prodTitle">كل المنتجات</h2><div class="'+(TH.gridClass||'pgrid')+'" id="mainGrid"><div class="skel"><div class="skel-i"><div class="skel-img"></div><div class="skel-t"></div><div class="skel-p"></div></div><div class="skel-i"><div class="skel-img"></div><div class="skel-t"></div><div class="skel-p"></div></div><div class="skel-i"><div class="skel-img"></div><div class="skel-t"></div><div class="skel-p"></div></div><div class="skel-i"><div class="skel-img"></div><div class="skel-t"></div><div class="skel-p"></div></div></div></div>></div></section>';
 ...
 return '';}
function render(){if(!TH)return;var tb=document.getElementById('toolbar');if(tb)tb.style.display=TH.hideToolbar?'none':'';
...
function renderGrid(){var e=document.getElementById('mainGrid');if(!e||!TH)return;var v=visIdx();var tt=document.getElementById('prodTitle');if(tt)tt.textContent=(curCat!=='الكل'?curCat:(curQ?'نتائج البحث':'كل المنتجات'))+' ('+v.length+')';var skel=e.querySelector('.skel');if(skel)skel.remove();e.innerHTML=v.map(function(i){return TH.card(P[i],i)}).join('')||'<div style="grid-column:1/-1;text-align:center;padding:30px;color:#999">لا توجد منتجات مطابقة 🔍</div>';}
...
COMM=20;
function startWith(data){P=data||[];boot('premium');}
if(window.__PRODUCTS&&window.__PRODUCTS.length){
  startWith(window.__PRODUCTS);
  // background refresh للتحديثات
  fetch('/api/products').then(function(r){return r.json()}).then(function(d){
    if(d&&d.length&&JSON.stringify(d)!==JSON.stringify(P)){P=d;render();postRender();}
  }).catch(function(){});
}else{
  fetch('/api/products').then(r=>r.json()).then(function(d){startWith(d)}).catch(function(){startWith([])});
}
...
function submitOrder(){var m=coMsg;m.className='msg';if(!fName.value||!fPhone.value||!fAddr.value){m.textContent='⚠️ اكمل بياناتك';m.className='msg err';return;}
 ...
 fetch('/api/create-order',{method:'POST',headers:{'Content-Type':'application/json','x-auth-token':localStorage.getItem('etok')||''},body:JSON.stringify({client_name:fName.value,client_phone1:fPhone.value,client_address:fAddr.value,shipping_governorate:fGov.value,items:items,productNames:cart.map(c=>c.name),total:tot,commission:COMM,shipping_cost:SH})}).then(r=>r.json()).then(function(d){if(d.error){m.textContent='⚠️ '+d.error;m.className='msg err';}else{m.textContent='✅ تم استلام طلبك';m.className='msg ok';cart=[];updCC();setTimeout(()=>closeM('co'),1400);}}).catch(function(){m.textContent='⚠️ خطأ';m.className='msg err';});}
