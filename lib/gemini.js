...
async function generateProductDescription(product) {
  ...
  for (let index = 0; index < modelsToTry.length; index += 1) {
    ...
    try {
      response = await fetch(geminiEndpoint(usedModel), {
        ...
        body: JSON.stringify(requestBodyFor(context)),
    ...
    throw geminiError(body, response.status);
  }
  ...
  return { description, model: usedModel };
}

module.exports = { GEMINI_MODEL, GEMINI_FALLBACK_MODEL, GEMINI_ENDPOINT, productContext, generateProductDescription };
