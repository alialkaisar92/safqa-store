...
const inline = context.compactAnswer('المنتجات المتاحة: - منتج 1 — سعر البيع: 100 ج.م — العمولة: 40 ج.م — متوفر - منتج 2 — سعر البيع: 120 ج.م — العمولة: 50 ج.م — متوفر - منتج 3 — سعر البيع: 130 ج.م — العمولة: 60 ج.م — متوفر', 420);
const inlineProductLines = inline.split('\n').filter(line => /^[-•]\s/.test(line));
if (inlineProductLines.length !== 3) throw new Error(`expected inline list to become 3 product lines, got ${inlineProductLines.length}`);
console.log('PASS compact answer removes literal newlines and emoji, keeps 3 product lines, and splits inline lists');
