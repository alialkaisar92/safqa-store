...
function storeStartCountdown(){clearInterval(storeResendTimer);storeResendUntil=Date.now()+60000;var b=document.getElementById('storeResendBtn');function tick(){var left=Math.max(0,Math.ceil((storeResendUntil-Date.now())/1000));if(!b)return;if(left){b.disabled=true;b.textContent='إعادة الإرسال بعد '+left+' ثانية'}else{b.disabled=false;b.textContent='إعادة إرسال الرمز';clearInterval(storeResendTimer)}}tick();storeResendTimer=setInterval(tick,1000)}
