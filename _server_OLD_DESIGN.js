...
body{font-family:Cairo,sans-serif;background:var(--bg);color:var(--text);padding-bottom:80px}
.header{background:rgba(255, 255, 255, 0.95);backdrop-filter:blur(10px);padding:14px 20px;position:sticky;top:0;z-index:100;display:flex;justify-content:space-between;align-items:center;box-shadow:0 4px 20px rgba(0,0,0,.04);border-bottom:1px solid #f1f5f9}
.logo{font-weight:900;font-size:1.6rem;color:#0f172a;letter-spacing:-0.5px;display:flex;align-items:center;gap:8px}
.logo span{color:var(--pd)}
.cart-btn{background:#f1f5f9;border:1.5px solid #e2e8f0;color:#0f172a;padding:8px 16px;border-radius:30px;font-weight:700;font-size:.9rem;cursor:pointer;font-family:Cairo,sans-serif;position:relative;display:flex;align-items:center;gap:6px;transition:all .2s}
.cart-btn:active{transform:scale(0.95)}
.cart-btn .badge{position:absolute;top:-5px;left:-5px;background:var(--accent);color:#fff;border-radius:50%;min-width:22px;height:22px;font-size:.75rem;display:flex;align-items:center;justify-content:center;font-weight:800;box-shadow:0 2px 8px rgba(245,158,11,.4)}
...
.search{width:100%;padding:14px 18px;border:1.5px solid #e2e8f0;border-radius:16px;font-family:Cairo,sans-serif;font-size:.95rem;margin-bottom:14px;box-shadow:0 2px 12px rgba(0,0,0,.02);outline:none;background:#fff;transition:all .2s}
.search:focus{border-color:var(--pd);box-shadow:0 0 0 4px rgba(13,148,136,.1)}
.cats{display:flex;gap:8px;overflow-x:auto;margin-bottom:16px;scrollbar-width:none;padding-bottom:4px}
.cats::-webkit-scrollbar{display:none}
.c{padding:8px 16px;border-radius:12px;border:1.5px solid #e2e8f0;background:#fff;font-family:Cairo,sans-serif;font-weight:700;font-size:.82rem;color:var(--muted);cursor:pointer;white-space:nowrap;transition:all .2s}
.c.active{background:var(--pd);border-color:var(--pd);color:#fff;box-shadow:0 4px 12px rgba(13,148,136,.25)}
.grid{display:grid;grid-template-columns:repeat(2,1fr);gap:14px}
@media(min-width:480px){.grid{grid-template-columns:repeat(3,1fr)}}
.card{background:var(--card);border-radius:18px;overflow:hidden;box-shadow:0 4px 15px rgba(0,0,0,.04);cursor:pointer;transition:transform .2s,box-shadow .2s;border:1px solid #f1f5f9}
.card:active{transform:scale(.97)}
.card img{width:100%;aspect-ratio:1;object-fit:cover;background:#f1f5f9}
.card .b{padding:12px}
.card .t{font-size:.85rem;font-weight:700;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;min-height:2.4em;margin-bottom:6px;line-height:1.35;color:#0f172a}
.card .pr{font-size:1rem;font-weight:800;color:var(--pd)}
.card .stock{font-size:.72rem;margin-top:6px;font-weight:600}
...
.pm-h{position:sticky;top:0;background:#ffffff;color:#111827;padding:14px 16px;display:flex;justify-content:space-between;align-items:center;z-index:5;border-bottom:1px solid #f1f5f9}
.pm-h h3{font-size:1.05rem;font-weight:700}
.pm-h button{width:38px;height:38px;border-radius:50%;background:#f1f5f9;border:none;color:#111827;font-size:1.4rem;cursor:pointer;display:flex;align-items:center;justify-content:center}
...
.btn-accent{background:linear-gradient(135deg,#f59e0b,#d97706);color:#fff;box-shadow:0 6px 20px rgba(245,158,11,.35)}
.btn-primary{background:var(--pd);color:#fff;box-shadow:0 6px 20px rgba(13,148,136,.3)}
...
.cart-item{display:flex;gap:12px;background:var(--card);border-radius:16px;padding:14px;margin-bottom:10px;box-shadow:0 2px 10px rgba(0,0,0,.04);align-items:flex-start;border:1px solid #f1f5f9}
...
.cart-footer{position:sticky;bottom:80px;background:#fff;padding:16px;border-radius:20px 20px 0 0;box-shadow:0 -8px 30px rgba(0,0,0,.08);margin:16px -16px -16px;border-top:1px solid #f1f5f9}
...
.order{background:var(--card);border-radius:16px;padding:14px;margin-bottom:10px;box-shadow:0 2px 10px rgba(0,0,0,.04);border:1px solid #f1f5f9}
...
.stat{background:var(--card);border-radius:16px;padding:16px;box-shadow:0 2px 10px rgba(0,0,0,.04);border:1px solid #f1f5f9}
...
.w{border:2px solid #e2e8f0;border-radius:14px;padding:12px;text-align:center;cursor:pointer;font-size:.8rem;font-weight:700;background:#fff}
...
<div class="page" id="p-profile">
  ...
  <button class="btn btn-primary" onclick="saveProf()" style="margin-top:14px">حفظ التعديلات</button>
...
<div class="page" id="p-withdraw">
  ...
  <div class="ws">
    <div class="w sel" data-m="vodafone" onclick="sw(this)"><div class="i" style="background:#e60000">V</div>فودافون كاش</div>
    <div class="w" data-m="instapay" onclick="sw(this)"><div class="i" style="background:#1e40af">IP</div>إنستاباي</div>
    <div class="w" data-m="orange" onclick="sw(this)"><div class="i" style="background:#ff7900">O</div>أورنج كاش</div>
    <div class="w" data-m="bank" onclick="sw(this)"><div class="i" style="background:#0f766e">ب</div>تحويل بنكي</div>
  </div>
  <label>المبلغ المطلوب</label><input type="number" id="wAmt" placeholder="المبلغ">
  <label>رقم المحفظة / الحساب</label><input id="wDet" placeholder="01xxxxxxxxx">
  <button class="btn btn-primary" onclick="doWd()" style="margin-top:14px">تأكيد طلب السحب</button>
...
<div class="page" id="p-support">
  ...
  <button class="btn btn-primary" onclick="sendSupport()" style="margin-top:14px">إرسال للدعم</button>
...
function stockLabel(s, avail){
  ...
  if(s<=5) return '<span class="stock-low">متبقي: '+s+' قطعة</span>';
