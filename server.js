...
function chatLoad(){try{return JSON.parse(require('fs').readFileSync(CHAT_FILE,'utf8'))}catch(e){return[]}}
app.get('/api/chat',(req,res)=>res.json(chatLoad()));
app.post('/api/chat',(req,res)=>{const b=req.body||{};const all=chatLoad();const m={id:Date.now(),from:b.from||'user',type:b.type||'text',text:b.text||'',data:b.data||'',time:new Date().toLocaleTimeString('ar-EG',{hour:'2-digit',minute:'2-digit'})};all.push(m);require('fs').writeFileSync(CHAT_FILE,JSON.stringify(all));res.json({ok:true,m});});
...
async function loadChat(){try{const d=await(await fetch('/api/chat')).json();const b=document.getElementById('waBody');if(!b)return;b.innerHTML=(d||[]).map(m=>{let inner='';if(m.type==='image')inner='<img src="'+m.data+'" style="max-width:180px;border-radius:10px">';else if(m.type==='audio')inner='<audio controls src="'+m.data+'" style="width:200px"></audio>';else inner='<span>'+(m.text||'').replace(/</g,'&lt;')+'</span>';return '<div class="wa-msg '+(m.from==='user'?'me':'them')+'">'+inner+'<small>'+m.time+'</small></div>';}).join('');b.scrollTop=b.scrollHeight;}catch(e){}}
async function waSend(){const t=document.getElementById('waText').value.trim();if(!t)return;await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({from:'user',type:'text',text:t})});document.getElementById('waText').value='';loadChat();}
document.getElementById('waFile').onchange=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=async()=>{await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({from:'user',type:'image',data:r.result})});loadChat();};r.readAsDataURL(f);e.target.value='';};
document.getElementById('waMic').onclick=async()=>{const btn=document.getElementById('waMic');if(waRec){waRec.stop();return;}try{const st=await navigator.mediaDevices.getUserMedia({audio:true});waRec=new MediaRecorder(st);waChunks=[];waRec.ondataavailable=e=>waChunks.push(e.data);waRec.onstop=async()=>{const blob=new Blob(waChunks,{type:'audio/webm'});const r=new FileReader();r.onload=async()=>{await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({from:'user',type:'audio',data:r.result})});loadChat();};r.readAsDataURL(blob);st.getTracks().forEach(x=>x.stop());waRec=null;btn.textContent='🎤';btn.classList.remove('rec');};waRec.start();btn.textContent='⏹';btn.classList.add('rec');}catch(e){alert('فعّل الميكروفون أولاً');}};
