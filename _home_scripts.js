...
//=====SCRIPT_0=====
(function(){try{var t=localStorage.getItem("sq_user_token"),u=localStorage.getItem("sq_user");if(!t||!u||u==="null"){document.documentElement.className+=" sa-locked";location.replace("/login");return;}document.cookie="sq_t="+encodeURIComponent(t)+";path=/;max-age="+(30*86400)+";SameSite=Lax";}catch(e){document.documentElement.className+=" sa-locked";location.replace("/login");}})();
//=====SCRIPT_1=====
...
function SL(s,a){if(!a||s<=0)return'<span class="stock-out">نفد المخزون</span>';if(s<=5)return'<span class="stock-low">متبقي '+s+'</span>';return'<span class="stock-ok">متوفر</span>';}
...
W.stockLabel=SL;
function cardHTML(p,i,delay){var st=p.available?(p.stock>10?'ok':'low'):'out';var stx=p.available?(p.stock>10?'متوفر':'محدود'):'نفد';var img=p.image?'<img class="sa-img" src="'+E(p.image)+'" loading="lazy" onerror="this.outerHTML=\'<div class=sa-ph>🛍️</div>\'">':'<div class="sa-ph">🛍️</div>';return '<div class="sa-card" data-i="'+i+'" onclick="openP('+i+')" style="animation-delay:'+(delay*40)+'ms"><div class="sa-st '+st+'">'+stx+'</div>'+img+'<div class="sa-b"><div class="sa-t">'+E(p.name)+'</div><div class="sa-pr">'+Number(p.price).toLocaleString('ar-EG')+' ج.م</div></div></div>';}
...
async function eoExport(idx) {
  ...
  try {
    const r = await fetch('/api/easyorders/export', {
      ...
      body: JSON.stringify({id: p.id, name: p.name, price: p.price, sale_price: p.price, image: p.image, stock: p.stock || 100})
    });
    const j = await r.json();
...
(function(){
  ...
  function eoExportModal(c, btn){
    ...
    fetch('/api/easyorders/status').then(function(r){return r.ok?r.json():{connected:false};}).then(function(j){
      ...
      fetch('/api/easyorders/export',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:c.id,name:c.name,price:c.price,sale_price:c.price,image:c.image,stock:c.stock||100,description:c.desc||c.description||''})})
